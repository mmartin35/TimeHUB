...
class EventCalendar(HTMLCalendar):
    ...
    def formatday(self, day, weekday):
        ...
        events_today = self.events.filter(date=day_date)  # Using 'date' here
...
def planning(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            date = form.cleaned_data['date']  # Using 'date' here
            ...
            for i in range(duration):
                ...
                Event.objects.create(
                    title=title,
                    date=event_date  # Save with 'date'
                )
            return redirect('planning')  # Redirect to the correct view
    else:
        form = EventForm()
